...
public:
    Node* cloneGraph(Node* node) {
        unordered_map<Node*, Node*> dummy;
        stack<Node *> stk;
        
        stk.push(node);
        dummy[node] = new Node(node->val);
        
        while(!stk.empty())
        {
            Node *org = stk.top();    stk.pop();
            Node *cpy = dummy[org];
            
            for(auto it: org->neighbors)
            {
                if(dummy.find(it) == dummy.end())
                {
                    ...
                    cpy->neighbors.push_back(dummy[it]);
            }
        }
